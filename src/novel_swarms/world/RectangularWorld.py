    ...
    def withinWorldBoundaries(self, agent: DifferentialDriveAgent):
        ...
        if isinstance(agent, HeroRobot):
            ...
            if agent.x_pos != old_x or agent.y_pos != old_y or agent.old_collide:
                ...
                wall_dir = np.cross(-collide_point_vector, np.array([agent.dx, agent.dy, 0]))[2] / (
                            2 * (agent.c_now[0] + 0.000000000000000001) ** 2)
                agent.angle -= (
                            agent.da + agent.da * -wall_dir)  #(-1+(((wall_dir*np.linalg.norm(np.array([agent.dx,agent.dy,0])))+agent.c_now[0]*np.sqrt(2))/(agent.c_now[0]*np.sqrt(2)))))

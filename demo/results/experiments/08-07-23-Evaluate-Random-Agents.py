...
MAX_ALLOWED_STEPS = 20000  # Maximum Steps allowed during simulation
...
def evaluate_randoms():
    ...
    for i in range(NUM_AGENTS):
        y_i_behaviors = [w.behavior[i].out_current()[1] for w in world_ret]
        print(y_i_behaviors)
        y_i = np.average(y_i_behaviors)

        y.append(y_i)
        std_err = 2.575829 * np.std(y_i_behaviors) / np.sqrt(len(y_i_behaviors))
        y_err.append(std_err)

        y_mins.append(min(y_i_behaviors))
        y_maxes.append(max(y_i_behaviors))

    x = [i for i in range(1, NUM_AGENTS + 1)]

    plt.plot(x, y)
    # plt.fill_between(x, [y[i] + y_err[i] for i in range(len(y))], [y[i] - y_err[i] for i in range(len(y))], alpha=0.2)
    plt.fill_between(x, [y_maxes[i] for i in range(len(y))], [y_mins[i] for i in range(len(y))], alpha=0.2)
    plt.ylim(bottom=0)
    plt.xlabel(f"Desired Number of Agents at the Goal ({NUM_AGENTS} Total)")
    plt.ylabel("Average Time to Goal (ticks) (Max/Min Lines Shown)")
    plt.title("Levy Agent's Average Time to Goal over 500 seeds")
    plt.show()
